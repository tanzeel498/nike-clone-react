...
function Collapsible({ children, initialExpand = true }) {
  const [expand, setExpand] = useState(initialExpand);
  const handleChange = () => setExpand((s) => !s);

  return (
    <CollapsibleContext.Provider value={{ expand, handleChange }}>
      {children}
...
function Trigger({ children }) {
  const { expand, handleChange } = useContext(CollapsibleContext);
  ...
  return (
    ...
    >
      ...
      <div className="flex translate-y-1 items-center">
        <svg viewBox="0 0 8 8" width="18px" height="14px" fill="#404040">
          <g className={`${expand ? "-translate-y-[3px]" : ""} duration-300`}>
            <path
              ...
              className={`chevron-left origin-center duration-300 ${
                expand ? "-rotate-90" : ""
              }`}
            />
            <path
              ...
              className={`chevron-right origin-center duration-300 ${
                expand ? "rotate-90" : ""
              }`}
            />
          </g>
...
function Group({ children }) {
  const { expand } = useContext(CollapsibleContext);
